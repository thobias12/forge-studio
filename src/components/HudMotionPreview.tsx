...
export function HudMotionPreviewLayer({ layout, motion }: { layout: SkillboundHudLayout; motion?: HudMotionEvent }) {
  ...
  return <div className={`hud-motion-layer motion-${motion.kind}`} key={`${motion.kind}-${motion.id}`}>
    {Array.from({ length: particles }).map((_, index) => {
      const spread = ((index % 5) - 2) * 1.15
      const sourceSpread = ((index % 3) - 1) * 1.25
      const midX = sourceX + (targetX - sourceX) * .46 + spread
      const midY = Math.min(sourceY, targetY) - 9 - (index % 3) * 2.2
      const style = {
        '--source-x': `${sourceX + sourceSpread}%`,
        '--source-y': `${sourceY + (index % 2) * 1.2}%`,
        '--mid-x': `${midX}%`,
        '--mid-y': `${midY}%`,
        '--target-x': `${targetX}%`,
        '--target-y': `${targetY}%`,
        '--flight-delay': `${index * 48}ms`,
      } as CSSProperties
