...
function makeGeneratedPath(region: GeneratedRegion, path: GeneratedWorldPath) {
  ...
  const road = new THREE.Mesh(
    makeRuntimeRibbon(path.points, widths, (x, z) => sampleTerrainHeight(region, x, z) + .054),
    new THREE.MeshStandardMaterial({
...
function addGeneratedPois(scene: THREE.Scene, region: GeneratedRegion, obstacles: CircleObstacle[]) {
  ...
  for (const poi of region.pois) {
    ...
    if (poi.type === 'ruins') {
      ...
      group.add(tower)
      for (let i = 0; i < 7; i += 1) {
        const angle = i / 7 * Math.PI * 2 + .35
        const rubble = new THREE.Mesh(new THREE.DodecahedronGeometry(.32 + (i % 3) * .1, 0), darkStone)
        rubble.position.set(Math.cos(angle) * (2.45 + (i % 2) * .45), .18, Math.sin(angle) * (2.45 + (i % 2) * .45))
        rubble.scale.y = .62
