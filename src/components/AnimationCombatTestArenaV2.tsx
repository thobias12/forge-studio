...
export default function AnimationCombatTestArenaV2({ target, animationSet, action, autoPlayToken = 0 }: Props) {
  ...
  useEffect(() => {
    ...
    const floor = new THREE.Mesh(new THREE.CircleGeometry(5.2, 48), new THREE.MeshStandardMaterial({ color: 0x17191c, roughness: .9, metalness: .08 }))
    floor.rotation.x = -Math.PI / 2
    floor.receiveShadow = true
    scene.add(floor)

    const ring = new THREE.Mesh(new THREE.RingGeometry(1.8, 1.86, 48), new THREE.MeshBasicMaterial({ color: 0x6f5133, transparent: true, opacity: .55, side: THREE.DoubleSide }))
    ...
    const dummyBase = new THREE.Mesh(new THREE.CylinderGeometry(.72, .82, .18, 24), new THREE.MeshStandardMaterial({ color: 0x26272a, roughness: .75, metalness: .4 }))
  ...
  }, [target?.id, animationSet?.updatedAt])
  ...
  function fireEvent(kind: 'hit' | 'vfx' | 'sfx' | 'recovery', assetId?: string) {
    if (kind === 'hit') {
      setEventLabel('HIT')
  ...
  useEffect(() => {
    if (!ready || autoPlayToken <= 0 || autoPlayToken === lastAutoTokenRef.current) return
    lastAutoTokenRef.current = autoPlayToken
    void trigger()
  }, [ready, autoPlayToken, action, binding?.clip])
  ...
  return <section className="animation-combat-arena animation-combat-arena-v2">
    <header>
      <div><span className="eyebrow">IN-STUDIO GAMEPLAY TEST</span><strong>{target?.name ?? 'Choose a character'}</strong><small>Publishes the selected draft, then immediately runs that exact action here.</small></div>
      <div className="animation-arena-actions"><button className="secondary-button" onClick={reset}><RotateCcw size={14}/> Reset</button><button className="primary-button" disabled={!ready} onClick={() => void trigger()}><Swords size={15}/> Replay {actionDefinition(action)?.label ?? 'Action'}</button></div>
    </header>
    <div className="animation-arena-body">
      <div className="animation-arena-stage" ref={hostRef}>
        {!target && <div className="animation-arena-empty">Choose a Library character before testing.</div>}
        {target && !ready && <div className="animation-arena-loading">Loading the published animation pack…</div>}
      ...
      <aside className="animation-arena-sequence">
        <strong>WHAT THE GAME WILL USE</strong>
        <div className="arena-sequence-clip"><span>0.00s</span><b>{binding?.clip ?? 'No published clip yet'}</b></div>
        ...
        {!events.length && <p>The animation itself is still testable. Add Hit / VFX / SFX timing below when you need gameplay events.</p>}
      </aside>
    </div>
  </section>
}
