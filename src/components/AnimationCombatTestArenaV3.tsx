...
export default function AnimationCombatTestArenaV3({ target, animationProfile, action }: Props) {
  ...
  useEffect(() => {
    let cancelled = false
    const binding = animationProfile?.actions[action]
    if (!binding?.assetId) {
      setAnimation(undefined)
      return
    }
    void getAsset(binding.assetId)
      .then((asset) => asset ? parseForgeAnimationV3(asset.blob) : undefined)
      .then((value) => { if (!cancelled) setAnimation(value) })
    return () => { cancelled = true }
  }, [animationProfile, action])
  ...
  useEffect(() => {
    ...
    if (!host || !target || !animationProfile) {
      setReady(false)
      return
    }
  ...
  }, [target?.id, animationProfile?.targetId])
  ...
  const trigger = async () => {
    ...
    const controller = bindingRef.current.getAnimationRuntimeV3()
    if (!controller?.playAction(action)) return

    for (const event of events) {
      const timer = window.setTimeout(
        () => fireEvent(event.kind, event.assetId),
  ...
  const reset = () => {
    setHealth(100)
    setEventLabel('Ready')
    bindingRef.current?.getAnimationRuntimeV3()?.setBase('idle', true)
  }

  return <section className="animation-combat-arena">
    <header>
      <div>
        <span className="eyebrow">COMBAT TEST ARENA · RUNTIME 3</span>
        <strong>{target?.name ?? 'Choose a character'}</strong>
        <small>Uses the same semantic animation controller as Play Project.</small>
      </div>
      <div className="animation-arena-actions">
        <button className="secondary-button" onClick={reset}><RotateCcw size={14}/> Reset</button>
        <button className="primary-button" disabled={!ready} onClick={() => void trigger()}><Swords size={15}/> Test {actionDefinition(action)?.label ?? 'Action'}</button>
      </div>
    </header>
    <div className="animation-arena-body">
      <div className="animation-arena-stage" ref={hostRef}>
        ...
        {target && !ready && <div className="animation-arena-loading">Loading body + Forge Animation Runtime v3…</div>}
      ...
      <aside className="animation-arena-sequence">
        ...
        <div className="arena-sequence-clip"><span>0.00s</span><b>{animation?.name ?? 'No authored v3 animation'}</b></div>
        ...
        {!events.length && <p>Runtime 3 action is published without extra event markers.</p>}
      </aside>
    </div>
  </section>
}
