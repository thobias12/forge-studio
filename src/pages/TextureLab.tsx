...
export default function TextureLab() {
  ...
  const importBase = async (file?: File) => {
    ...
    } catch (error) { setStatus(error instanceof Error ? error.message : 'Could not import that texture.') }
  ...
  const replaceChannel = async (channel: TextureChannel, file?: File) => {
    ...
    } catch (error) { setStatus(error instanceof Error ? error.message : 'Could not load that channel image.') }
    finally { setBusy(false) }
  }
  ...
  const generateMaps = async () => {
    ...
    } catch (error) { setStatus(error instanceof Error ? error.message : 'PBR generation failed.') }
    finally { setBusy(false) }
  }

  const exportChannel = (channel: TextureChannel) => {
    const blob = channels[channel]
    if (blob) downloadBlob(blob, `${safeName(name)}-${channel}.png`)
  }
  ...
  const saveMaterial = async () => {
    ...
    try {
      ...
      for (const channel of channelInfo) {
        ...
        if (!blob) continue
        const asset = await saveAsset({ name: `${name} ${channel.label}`, category: 'textures', kind: 'image', mime: 'image/png', tags: ['pbr', safeName(name), channel.id], source: `Texture Lab · ${name}`, blob })
        savedChannels[channel.id] = asset.id
      }
      const manifest = { format: 'forge-material', version: 1, name, createdAt: new Date().toISOString(), channels: savedChannels, parameters: { repeat, roughness, metalness, normalStrength } }
      await saveAsset({ name, category: 'materials', kind: 'file', mime: 'application/json', tags: ['pbr', 'material', safeName(name)], source: 'Forge Texture Lab', blob: new Blob([JSON.stringify(manifest, null, 2)], { type: 'application/json' }) })
      setStatus(`${name} saved with ${Object.keys(savedChannels).length} texture maps. It is now available in Shared Asset Library.`)
    } catch (error) { setStatus(error instanceof Error ? error.message : 'Could not save this material.') }
    finally { setBusy(false) }
  }
  ...
  return (
    <div className="texture-lab">
      <section className="texture-workspace">
        ...
        <div className="texture-channel-strip">{channelInfo.map((channel) => <button key={channel.id} className={`texture-channel-card ${activeChannel === channel.id ? 'active' : ''}`} onClick={() => setActiveChannel(channel.id)}><div className="texture-channel-thumb">{urls[channel.id] ? <img src={urls[channel.id]} alt="" /> : <Layers3 size={20} />}</div><div><strong>{channel.label}</strong><span>{channels[channel.id] ? channel.hint : 'Not assigned'}</span></div><i className={channels[channel.id] ? 'ready' : ''} /></button>)}</div>
      </section>
      <aside className="texture-inspector">
        <div className="inspector-heading"><Layers3 size={15} /> Material Inspector</div>
        <div className="texture-inspector-block"><label className="property-label">Material name</label><input className="texture-text-input" value={name} onChange={(event) => setName(event.target.value)} /><div className="texture-meta-row"><span>Resolution</span><strong>{resolution}</strong></div><div className="texture-meta-row"><span>Maps</span><strong>{populatedCount}/4</strong></div></div>
        <div className="texture-inspector-block"><span className="property-label">Material response</span><TextureSlider label="Tiling" value={repeat} min={0.25} max={8} step={0.25} display={`${repeat.toFixed(2)}×`} onChange={setRepeat} /><TextureSlider label="Roughness" value={roughness} min={0} max={1} step={0.01} display={`${Math.round(roughness * 100)}%`} onChange={setRoughness} /><TextureSlider label="Metalness" value={metalness} min={0} max={1} step={0.01} display={`${Math.round(metalness * 100)}%`} onChange={setMetalness} /><TextureSlider label="Normal strength" value={normalStrength} min={0} max={2.5} step={0.05} display={normalStrength.toFixed(2)} onChange={setNormalStrength} /><button className="texture-regenerate" disabled={!hasBase || busy} onClick={() => void generateMaps()}><RefreshCw size={14} /> Regenerate generated maps</button></div>
        <div className="texture-inspector-block channel-inspector"><div className="property-title"><span>{channelInfo.find((item) => item.id === activeChannel)?.label.toUpperCase()}</span><em>{channels[activeChannel] ? 'READY' : 'EMPTY'}</em></div><div className="channel-large-preview">{urls[activeChannel] ? <img src={urls[activeChannel]} alt="" /> : <Layers3 size={32} />}</div><div className="channel-actions"><label className="secondary-button file-button"><FileUp size={14} /> Replace<input type="file" accept="image/png,image/jpeg,image/webp" onChange={(event) => void replaceChannel(activeChannel, event.target.files?.[0])} /></label><button className="secondary-button" disabled={!channels[activeChannel]} onClick={() => exportChannel(activeChannel)}><Download size={14} /> PNG</button></div></div>
...
function safeName(value: string) { return value.trim().toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '') || 'forge-material' }
function downloadBlob(blob: Blob, filename: string) { const url = URL.createObjectURL(blob); const anchor = document.createElement('a'); anchor.href = url; anchor.download = filename; document.body.appendChild(anchor); anchor.click(); anchor.remove(); window.setTimeout(() => URL.revokeObjectURL(url), 1500) }
