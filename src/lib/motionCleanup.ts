...
type HandKey = 'leftHandLandmarks' | 'rightHandLandmarks' | 'leftHandWorldLandmarks' | 'rightHandWorldLandmarks'
...
function cloneFrame(frame: PoseFrame): PoseFrame {
  return {
    ...
    leftHandLandmarks: frame.leftHandLandmarks?.map(clonePoint),
    rightHandLandmarks: frame.rightHandLandmarks?.map(clonePoint),
    leftHandWorldLandmarks: frame.leftHandWorldLandmarks?.map(clonePoint),
    rightHandWorldLandmarks: frame.rightHandWorldLandmarks?.map(clonePoint),
  }
}

function isValid(point: PosePoint | undefined) {
  return !!point && Number.isFinite(point.x) && Number.isFinite(point.y) && Number.isFinite(point.z) && (point.visibility ?? 1) >= MIN_VISIBILITY
}
...
function getChannel(frame: PoseFrame, world: boolean) { return world ? frame.worldLandmarks : frame.landmarks }
  ...
  for (let frameIndex = 0; frameIndex < frames.length; frameIndex += 1) {
    ...
    for (let pointIndex = 0; pointIndex < count; pointIndex += 1) {
      ...
      for (let offset = -radius; offset <= radius; offset += 1) {
        ...
        sumX += sample!.x * weight; sumY += sample!.y * weight; sumZ += sample!.z * weight; totalWeight += weight
      }
      if (totalWeight <= 0) continue
      const average: PosePoint = { x: sumX / totalWeight, y: sumY / totalWeight, z: sumZ / totalWeight, visibility: original!.visibility }
      ...
      const motionScale = world ? Math.max(0.28, Math.min(1, 1 - motion / 0.22)) : Math.max(0.34, Math.min(1, 1 - motion / 0.12))
      target[pointIndex] = lerpPoint(original!, average, amount * motionScale)
      target[pointIndex].visibility = original!.visibility
    }
  }
}

function smoothHands(frames: PoseFrame[], key: HandKey, strength: number) {
  const amount = Math.max(0, Math.min(0.72, strength * 0.62))
  if (amount <= 0 || frames.length < 2) return
  let previous: PosePoint[] | undefined
  for (const frame of frames) {
    const current = frame[key]
    if (!current?.length) { previous = undefined; continue }
    if (previous?.length === current.length) {
      frame[key] = current.map((point, index) => {
        const before = previous![index]
        return { x: before.x + (point.x - before.x) * (1 - amount), y: before.y + (point.y - before.y) * (1 - amount), z: before.z + (point.z - before.z) * (1 - amount), visibility: point.visibility }
      })
    }
    previous = frame[key]?.map(clonePoint)
  }
}
...
function applyOffset(points: PosePoint[] | undefined, indices: number[], dx: number, dy: number, dz: number, amount: number) {
  ...
  for (const index of indices) {
    ...
    point.x += dx * amount; point.y += dy * amount; point.z += dz * amount
  }
}
...
function stabilizeFeet(frames: PoseFrame[], world: boolean, strength: number, footLock: boolean, groundAlign: boolean) {
  ...
  for (const centers of centersBySide) for (const center of centers) if (center) floorSamples.push(center.y)
  ...
  const locked = new Set<number>(), aligned = new Set<number>()
  ...
  centersBySide.forEach((centers, side) => {
    ...
    for (let index = 0; index < frames.length; index += 1) {
      ...
      plantedFrames = planted ? plantedFrames + 1 : 0
      if (groundAlign && center && nearFloor) {
        applyOffset(getChannel(frames[index], world), indices, 0, floor - center.y, 0, alignAmount)
        aligned.add(index)
      }
      if (!footLock || !center || plantedFrames < 2) { if (!planted) anchor = undefined; continue }
      if (!anchor) anchor = { ...center, y: groundAlign ? floor : center.y }
      applyOffset(getChannel(frames[index], world), indices, anchor.x - center.x, anchor.y - center.y, anchor.z - center.z, lockAmount)
      locked.add(index)
    }
  })
  return { footLockedFrames: locked.size, groundAlignedFrames: aligned.size }
}
...
export function cleanupMotion(motion: ForgeMotion, options: MotionCleanupOptions) {
  ...
  smoothHands(frames, 'leftHandLandmarks', strength)
  smoothHands(frames, 'rightHandLandmarks', strength)
  smoothHands(frames, 'leftHandWorldLandmarks', strength)
  smoothHands(frames, 'rightHandWorldLandmarks', strength)

  const hasWorld = frames.some((frame) => frame.worldLandmarks?.length === 33)
  const worldFoot = hasWorld ? stabilizeFeet(frames, true, strength, options.footLock, options.groundAlign) : { footLockedFrames: 0, groundAlignedFrames: 0 }
  const screenFoot = stabilizeFeet(frames, false, strength * 0.75, options.footLock, options.groundAlign)
  const cleaned: ForgeMotion = { ...motion, frames }
