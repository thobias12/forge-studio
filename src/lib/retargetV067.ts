...
type BoneAimState = { bone: THREE.Bone; restLocalQuaternion: THREE.Quaternion; axisLocal: THREE.Vector3 }
type BoneBasisState = { bone: THREE.Bone; sourceBasisQuaternion: THREE.Quaternion }
type HandTrackingState = {
  palmLocal?: THREE.Quaternion
  palmForward?: THREE.Vector3
  palmAcross?: THREE.Vector3
  flex: Partial<Record<FingerBoneKey, number>>
  spread: Partial<Record<FingerBoneKey, number>>
}
...
export type RetargetRuntime = {
  ...
  handTracking: { left: HandTrackingState; right: HandTrackingState }
...
const aliases: Partial<Record<HumanoidBoneKey, string[]>> = {
  hips: ['hips', 'pelvis', 'hip', 'rootpelvis'], spine: ['spine', 'spine01', 'spine1', 'lowerback', 'abdomen'], chest: ['chest', 'upperchest', 'spine02', 'spine2', 'spine03', 'spine3', 'upperbody'], neck: ['neck', 'neck01', 'neck1'], head: ['head', 'head01'],
  leftUpperArm: ['leftupperarm', 'leftarm', 'upperarml', 'arml', 'lupperarm'], leftLowerArm: ['leftlowerarm', 'leftforearm', 'lowerarml', 'forearml', 'llowerarm'], leftHand: ['lefthand', 'handl', 'lhand'],
  rightUpperArm: ['rightupperarm', 'rightarm', 'upperarmr', 'armr', 'rupperarm'], rightLowerArm: ['rightlowerarm', 'rightforearm', 'lowerarmr', 'forearmr', 'rlowerarm'], rightHand: ['righthand', 'handr', 'rhand'],
  leftUpperLeg: ['leftupperleg', 'leftupleg', 'leftthigh', 'upperlegl', 'thighl', 'lupperleg'], leftLowerLeg: ['leftlowerleg', 'leftleg', 'leftshin', 'leftcalf', 'lowerlegl', 'shinl', 'calfl', 'llowerleg'], leftFoot: ['leftfoot', 'footl', 'lfoot', 'leftankle'], leftToes: ['lefttoebase', 'lefttoe', 'toel', 'ltoe', 'lefttoes'],
  rightUpperLeg: ['rightupperleg', 'rightupleg', 'rightthigh', 'upperlegr', 'thighr', 'rupperleg'], rightLowerLeg: ['rightlowerleg', 'rightleg', 'rightshin', 'rightcalf', 'lowerlegr', 'shinr', 'calfr', 'rlowerleg'], rightFoot: ['rightfoot', 'footr', 'rfoot', 'rightankle'], rightToes: ['righttoebase', 'righttoe', 'toer', 'rtoe', 'righttoes'],
  leftThumb1: ['lefthandthumb1', 'leftthumb1', 'thumb01l', 'lthumb1'], leftThumb2: ['lefthandthumb2', 'leftthumb2', 'thumb02l', 'lthumb2'], leftThumb3: ['lefthandthumb3', 'leftthumb3', 'thumb03l', 'lthumb3'],
  leftIndex1: ['lefthandindex1', 'leftindex1', 'index01l', 'lindex1'], leftIndex2: ['lefthandindex2', 'leftindex2', 'index02l', 'lindex2'], leftIndex3: ['lefthandindex3', 'leftindex3', 'index03l', 'lindex3'],
  leftMiddle1: ['lefthandmiddle1', 'leftmiddle1', 'middle01l', 'lmiddle1'], leftMiddle2: ['lefthandmiddle2', 'leftmiddle2', 'middle02l', 'lmiddle2'], leftMiddle3: ['lefthandmiddle3', 'leftmiddle3', 'middle03l', 'lmiddle3'],
  leftRing1: ['lefthandring1', 'leftring1', 'ring01l', 'lring1'], leftRing2: ['lefthandring2', 'leftring2', 'ring02l', 'lring2'], leftRing3: ['lefthandring3', 'leftring3', 'ring03l', 'lring3'],
  leftPinky1: ['lefthandpinky1', 'lefthandlittle1', 'leftpinky1', 'leftlittle1', 'pinky01l', 'little01l'], leftPinky2: ['lefthandpinky2', 'lefthandlittle2', 'leftpinky2', 'leftlittle2', 'pinky02l', 'little02l'], leftPinky3: ['lefthandpinky3', 'lefthandlittle3', 'leftpinky3', 'leftlittle3', 'pinky03l', 'little03l'],
  rightThumb1: ['righthandthumb1', 'rightthumb1', 'thumb01r', 'rthumb1'], rightThumb2: ['righthandthumb2', 'rightthumb2', 'thumb02r', 'rthumb2'], rightThumb3: ['righthandthumb3', 'rightthumb3', 'thumb03r', 'rthumb3'],
  rightIndex1: ['righthandindex1', 'rightindex1', 'index01r', 'rindex1'], rightIndex2: ['righthandindex2', 'rightindex2', 'index02r', 'rindex2'], rightIndex3: ['righthandindex3', 'rightindex3', 'index03r', 'rindex3'],
  rightMiddle1: ['righthandmiddle1', 'rightmiddle1', 'middle01r', 'rmiddle1'], rightMiddle2: ['righthandmiddle2', 'rightmiddle2', 'middle02r', 'rmiddle2'], rightMiddle3: ['righthandmiddle3', 'rightmiddle3', 'middle03r', 'rmiddle3'],
  rightRing1: ['righthandring1', 'rightring1', 'ring01r', 'rring1'], rightRing2: ['righthandring2', 'rightring2', 'ring02r', 'rring2'], rightRing3: ['righthandring3', 'rightring3', 'ring03r', 'rring3'],
  rightPinky1: ['righthandpinky1', 'righthandlittle1', 'rightpinky1', 'rightlittle1', 'pinky01r', 'little01r'], rightPinky2: ['righthandpinky2', 'righthandlittle2', 'rightpinky2', 'rightlittle2', 'pinky02r', 'little02r'], rightPinky3: ['righthandpinky3', 'righthandlittle3', 'rightpinky3', 'rightlittle3', 'pinky03r', 'little03r'],
}

const aimChildren: Partial<Record<HumanoidBoneKey, HumanoidBoneKey>> = {
  neck: 'head', leftUpperArm: 'leftLowerArm', leftLowerArm: 'leftHand', rightUpperArm: 'rightLowerArm', rightLowerArm: 'rightHand',
  ...
  leftThumb1: 'leftThumb2', leftThumb2: 'leftThumb3', leftIndex1: 'leftIndex2', leftIndex2: 'leftIndex3', leftMiddle1: 'leftMiddle2', leftMiddle2: 'leftMiddle3', leftRing1: 'leftRing2', leftRing2: 'leftRing3', leftPinky1: 'leftPinky2', leftPinky2: 'leftPinky3',
  rightThumb1: 'rightThumb2', rightThumb2: 'rightThumb3', rightIndex1: 'rightIndex2', rightIndex2: 'rightIndex3', rightMiddle1: 'rightMiddle2', rightMiddle2: 'rightMiddle3', rightRing1: 'rightRing2', rightRing2: 'rightRing3', rightPinky1: 'rightPinky2', rightPinky2: 'rightPinky3',
}

const terminalFingerKeys = FINGER_BONE_KEYS.filter((key) => key.endsWith('3'))

function normalizeBoneName(value: string) { return value.toLowerCase().replace(/mixamorig/g, '').replace(/bip001/g, '').replace(/bip01/g, '').replace(/armature/g, '').replace(/[^a-z0-9]/g, '') }
...
export function mapHumanoidRig(root: THREE.Object3D) {
  ...
  for (const key of HUMANOID_BONE_KEYS) {
    let best: THREE.Bone | undefined, bestScore = 0
  ...
  return { rig, info: { totalBones: bones.length, mappedCount: Object.keys(mapped).length, coreMappedCount, fingerMappedCount, coreTotal: CORE_BONE_KEYS.length, mapped, missing: CORE_BONE_KEYS.filter((key) => !rig[key]) } satisfies RigInfo }
}

function localDirectionFromWorld(bone: THREE.Bone, worldDirection: THREE.Vector3) { return worldDirection.clone().applyQuaternion(bone.getWorldQuaternion(new THREE.Quaternion()).invert()).normalize() }
...
export function createRetargetRuntime(root: THREE.Object3D): RetargetRuntime {
  ...
  return { root, rig, info, aims, bases, handTracking: { left: { flex: {}, spread: {} }, right: { flex: {}, spread: {} } } }
}

function finite(point: PosePoint | undefined, threshold = 0.2, ignoreVisibility = false) {
  return !!point && Number.isFinite(point.x) && Number.isFinite(point.y) && Number.isFinite(point.z) && (ignoreVisibility || (point.visibility ?? 1) >= threshold)
}
function cameraVector(point: PosePoint, mirrorX: boolean) { return new THREE.Vector3(mirrorX ? -point.x : point.x, -point.y, -point.z) }
function pointsToVectors(points: PosePoint[], mirrorX: boolean) { return points.map((point) => cameraVector(point, mirrorX)) }
function midpoint(a: THREE.Vector3, b: THREE.Vector3) { return a.clone().add(b).multiplyScalar(0.5) }

function desiredBasisLocal(state: BoneBasisState | undefined, rightWorld: THREE.Vector3, upWorld: THREE.Vector3) {
  if (!state || rightWorld.lengthSq() < 1e-8 || upWorld.lengthSq() < 1e-8) return undefined
  ...
  const targetBasis = makeBasisQuaternion(rightWorld.clone().applyQuaternion(inverseParent), upWorld.clone().applyQuaternion(inverseParent))
  return targetBasis?.multiply(state.sourceBasisQuaternion.clone().invert())
}
function orientBasis(runtime: RetargetRuntime, state: BoneBasisState | undefined, rightWorld: THREE.Vector3, upWorld: THREE.Vector3, blend: number) {
  const desired = desiredBasisLocal(state, rightWorld, upWorld)
...
function filteredDirection(previous: THREE.Vector3 | undefined, target: THREE.Vector3, alpha: number) {
  const normalized = target.clone().normalize()
  if (!previous || previous.lengthSq() < 1e-8) return normalized
  const dot = THREE.MathUtils.clamp(previous.dot(normalized), -1, 1)
  if (dot < -0.25) return previous.clone()
  const angle = Math.acos(dot)
  const maxStep = THREE.MathUtils.degToRad(28)
  const stepTarget = angle > maxStep ? previous.clone().applyQuaternion(new THREE.Quaternion().setFromAxisAngle(new THREE.Vector3().crossVectors(previous, normalized).normalize(), maxStep)) : normalized
  return previous.clone().lerp(stepTarget, alpha).normalize()
}

function stabilizePalmQuaternion(previous: THREE.Quaternion | undefined, desired: THREE.Quaternion) {
  if (!previous) return desired.clone()
  const next = desired.clone()
  if (previous.dot(next) < 0) next.set(-next.x, -next.y, -next.z, -next.w)
  const angle = previous.angleTo(next)
  const maxStep = angle > THREE.MathUtils.degToRad(95) ? THREE.MathUtils.degToRad(8) : angle > THREE.MathUtils.degToRad(55) ? THREE.MathUtils.degToRad(18) : THREE.MathUtils.degToRad(34)
  if (angle < 1e-5) return next
  const limited = previous.clone().slerp(next, Math.min(1, maxStep / angle))
  return previous.clone().slerp(limited, 0.72)
}

function smoothAngle(store: Partial<Record<FingerBoneKey, number>>, key: FingerBoneKey, target: number, maxStepDegrees: number, alpha: number) {
  const previous = store[key]
  if (previous === undefined) { store[key] = target; return target }
  const maxStep = THREE.MathUtils.degToRad(maxStepDegrees)
  const delta = THREE.MathUtils.clamp(target - previous, -maxStep, maxStep)
  const next = THREE.MathUtils.lerp(previous, previous + delta, alpha)
  store[key] = next
  return next
}

function segmentAngles(direction: THREE.Vector3, forward: THREE.Vector3, across: THREE.Vector3, normal: THREE.Vector3, spreadLimit: number, flexLimit: number) {
  const dir = direction.clone().normalize()
  const f = dir.dot(forward), a = dir.dot(across), n = dir.dot(normal)
  const spread = THREE.MathUtils.clamp(Math.atan2(a, Math.max(0.08, Math.sqrt(f * f + n * n))), -spreadLimit, spreadLimit)
  const flex = THREE.MathUtils.clamp(Math.atan2(n, Math.max(0.08, Math.sqrt(f * f + a * a))), -flexLimit, flexLimit)
  return { spread, flex }
}

function synthFingerDirection(forward: THREE.Vector3, across: THREE.Vector3, normal: THREE.Vector3, spread: number, flex: number) {
  const planar = forward.clone().multiplyScalar(Math.cos(spread)).add(across.clone().multiplyScalar(Math.sin(spread))).normalize()
  return planar.multiplyScalar(Math.cos(flex)).add(normal.clone().multiplyScalar(Math.sin(flex))).normalize()
}
...
function applyHand(runtime: RetargetRuntime, side: 'left' | 'right', points: PosePoint[] | undefined, mirrorX: boolean, blend: number) {
  ...
  if (![0, 5, 9, 17].every((index) => finite(points[index], 0, true))) return

  const handState = runtime.handTracking[side]
  const rawForward = p[9].clone().sub(p[0])
  const rawAcross = p[5].clone().sub(p[17])
  if (rawForward.lengthSq() < 1e-8 || rawAcross.lengthSq() < 1e-8) return

  handState.palmForward = filteredDirection(handState.palmForward, rawForward, 0.46)
  const acrossOrtho = rawAcross.clone().addScaledVector(handState.palmForward, -rawAcross.dot(handState.palmForward))
  handState.palmAcross = filteredDirection(handState.palmAcross, acrossOrtho, 0.42)
  const forward = handState.palmForward.clone().normalize()
  const across = handState.palmAcross.clone().addScaledVector(forward, -handState.palmAcross.dot(forward)).normalize()
  let normal = new THREE.Vector3().crossVectors(across, forward).normalize()
  if (normal.lengthSq() < 1e-8) normal = new THREE.Vector3(0, 0, 1)

  const basis = runtime.bases[side === 'left' ? 'leftHand' : 'rightHand']
  const desiredPalm = desiredBasisLocal(basis, across, forward)
  if (basis && desiredPalm) {
    handState.palmLocal = stabilizePalmQuaternion(handState.palmLocal, desiredPalm)
    basis.bone.quaternion.slerp(handState.palmLocal, Math.min(0.72, blend * 0.82))
    runtime.root.updateMatrixWorld(true)
  }

  const prefix = side === 'left' ? 'left' : 'right'
  const fingers: Array<[string, number, number, number, number, number, number]> = [
    ['Thumb', 1, 2, 2, 3, 3, 4], ['Index', 5, 6, 6, 7, 7, 8], ['Middle', 9, 10, 10, 11, 11, 12], ['Ring', 13, 14, 14, 15, 15, 16], ['Pinky', 17, 18, 18, 19, 19, 20],
  ]

  for (const [name, a1, b1, a2, b2, a3, b3] of fingers) {
    const pairs: Array<[number, number, 1 | 2 | 3]> = [[a1, b1, 1], [a2, b2, 2], [a3, b3, 3]]
    const thumb = name === 'Thumb'
    const firstRaw = p[b1].clone().sub(p[a1])
    const firstAngles = segmentAngles(firstRaw, forward, across, normal, THREE.MathUtils.degToRad(thumb ? 72 : 34), THREE.MathUtils.degToRad(thumb ? 78 : 92))
    const baseKey = `${prefix}${name}1` as FingerBoneKey
    const stableSpread = smoothAngle(handState.spread, baseKey, firstAngles.spread, 10, 0.58)

    for (const [a, b, joint] of pairs) {
      if (!finite(points[a], 0, true) || !finite(points[b], 0, true)) continue
      const key = `${prefix}${name}${joint}` as FingerBoneKey
      const raw = p[b].clone().sub(p[a])
      const maxFlex = THREE.MathUtils.degToRad(thumb ? (joint === 1 ? 78 : 96) : joint === 1 ? 95 : joint === 2 ? 118 : 105)
      const angles = segmentAngles(raw, forward, across, normal, THREE.MathUtils.degToRad(thumb ? 72 : 34), maxFlex)
      const stableFlex = smoothAngle(handState.flex, key, angles.flex, joint === 1 ? 18 : 22, 0.66)
      const spread = joint === 1 ? stableSpread : stableSpread * 0.18
      const direction = synthFingerDirection(forward, across, normal, spread, stableFlex)
      aimBone(runtime, key, direction, thumb ? Math.min(0.58, blend * 0.68) : Math.min(0.72, blend * 0.84), thumb ? 95 : 125)
    }
  }
}
