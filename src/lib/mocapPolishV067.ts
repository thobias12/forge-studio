...
export function polishPoseFrame(
  ...
  const ikKneeCount = recoverOccludedKnees(body, source, state, state.calibration)
  ...
  } else if (state.lastLeftHand && now - state.lastLeftHandAt <= config.dropoutHoldMs + 110) {
    leftHandHeld = true
  }
  ...
  } else if (state.lastRightHand && now - state.lastRightHandAt <= config.dropoutHoldMs + 110) {
    rightHandHeld = true
  }
...
function recoverOccludedKnees(points: PosePoint[], source: PosePoint[], state: MocapPolishState, calibration?: Calibration) {
  if (!calibration) return 0
  let count = 0
  count += recoverKnee(points, source, state, 23, 25, 27, calibration.upperLeg, calibration.lowerLeg) ? 1 : 0
  count += recoverKnee(points, source, state, 24, 26, 28, calibration.upperLeg, calibration.lowerLeg) ? 1 : 0
  return count
}

function recoverKnee(
  points: PosePoint[],
  source: PosePoint[],
  state: MocapPolishState,
  hipIndex: number,
  ...
  if (usable(source[kneeIndex], 0.24)) return false
  ...
  const knee = { ...chosen, visibility: 0.46 }
  points[kneeIndex] = knee
  state.bodyMemory[kneeIndex] = { point: clonePoint(knee), seenAt: state.lastTimestamp }
  return true
}
...
function stabilizeHinges(points: PosePoint[], state: MocapPolishState, space: MocapBodySpace, dt: number, strength: number) {
  ...
  for (const [name, a, b, c] of hingeTriples) {
    ...
    if (previousNormal && vectorLength(currentNormal) > 1e-6) {
      ...
      const violentFlip = normalDot < -0.2 && dt < 0.12
      const jump = movement > jumpThreshold
      if (violentFlip || jump) {
        const keep = Math.max(0.18, Math.min(0.74, strength * (violentFlip ? 0.86 : 0.6)))
...
function applySegmentLengthConstraints(points: PosePoint[], calibration: Calibration | undefined, strength: number) {
  ...
  const amount = Math.max(0.12, Math.min(0.7, strength * 0.64))
...
function constrainPair(points: PosePoint[], parentIndex: number, childIndex: number, targetLength: number, amount: number) {
  ...
  if (ratio > 0.78 && ratio < 1.23) return
...
function updateFootLock(
  ...
  const alpha = 1 - Math.exp(-dt * (space === 'world' ? 9 : 12))
  ...
  const nearTolerance = space === 'world' ? 0.09 : 0.04
  const liftTolerance = space === 'world' ? 0.115 : 0.05
  const plantRadius = space === 'world' ? 0.042 : 0.018
  const releaseRadius = space === 'world' ? 0.105 : 0.045
  const nearFloor = Math.abs(filtered.y - floorY) <= nearTolerance
  const lifted = filtered.y < floorY - liftTolerance

  if (state.locked && state.anchor) {
    const drift = horizontalDistance(filtered, state.anchor)
    if (lifted || drift > releaseRadius) state.releaseFrames += 1
    else state.releaseFrames = 0
  ...
  if (!state.locked) {
    ...
    } else if (horizontalDistance(filtered, state.candidateAnchor) <= plantRadius) {
      state.candidateMs += dt * 1000
      state.candidateAnchor = lerpPoint(state.candidateAnchor, filtered, 0.08)
    } else {
      state.candidateAnchor = clonePoint(filtered)
      state.candidateMs = 0
    }

    if (state.candidateAnchor && state.candidateMs >= 170) {
      state.locked = true
      state.anchor = { ...state.candidateAnchor, y: floorY }
      state.releaseFrames = 0
    }
  }

  if (!state.locked || !state.anchor) return
  const lockAmount = Math.max(0.5, Math.min(0.97, strength))
  const delta = {
    x: state.anchor.x - filtered.x,
    y: floorY - filtered.y,
    z: state.anchor.z - filtered.z,
  ...
  for (const index of indices) {
    ...
    points[index] = {
      ...
      y: point.y + delta.y * Math.min(0.94, lockAmount),
  ...
  if (knee) {
    points[kneeIndex] = {
      ...knee,
      x: knee.x + delta.x * 0.1 * lockAmount,
      y: knee.y + delta.y * 0.04 * lockAmount,
      z: knee.z + delta.z * 0.1 * lockAmount,
    }
  }
}
...
function stabilizeHand(source: PosePoint[] | undefined, previous: PosePoint[] | undefined, bodySpace: MocapBodySpace, strength: number) {
  ...
  const moveScale = bodySpace === 'world' ? 0.028 : 0.02
  ...
  return source.map((point, index) => {
    ...
    const baseAlpha = tipIndices.has(index) ? 0.28 : 0.38
    const alpha = Math.min(0.88, baseAlpha + responsive * (0.58 - strength * 0.1))
    return {
...
function horizontalDistance(a: PosePoint, b: PosePoint) {
  const dx = a.x - b.x
  const dz = a.z - b.z
