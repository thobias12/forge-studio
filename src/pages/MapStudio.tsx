...
import {
  ...
  type DungeonEncounter, type DungeonMarker, type DungeonRoom, type DungeonRoomType, type DungeonTheme, type DungeonTriggerAction, type ForgeDungeonPackage,
...
export default function MapStudio() {
  ...
  const onGroundClick = ({ x, z }: { x: number; z: number }) => {
    ...
    if (tool === 'room') {
      ...
      setSelectedRoomId(nextRoom.id); setSelectedMarkerId(undefined); setSelectedPropId(undefined); return
    }
  ...
  const onMarkerClick = (markerId: string) => { if (tool === 'erase') { mutate(removeMarkerReference(value, markerId), 'Marker removed.'); setSelectedMarkerId(undefined); return } setSelectedMarkerId(markerId); setSelectedRoomId(undefined); setSelectedPropId(undefined); setTool('select') }
  ...
  const proceduralGenerate = () => { const next: DungeonWithProps = { ...generateDungeon(Math.floor(Math.random() * 999999), value.theme), props: [] }; mutate(next, `${titleCase(value.theme)} dungeon generated with encounter flow.`); clearSelection(); setSelectedRoomId(next.rooms[0]?.id); setCorridorStartId(undefined) }
  ...
  return <div className="map-studio-page">
    <aside className="map-left-panel">
      ...
      <section className="map-panel-section compact"><div className="map-section-title">THEME + LOGIC</div><select value={value.theme} onChange={(e)=>mutate({...value,theme:e.target.value as DungeonTheme})}>{themes.map((item)=><option key={item} value={item}>{titleCase(item)}</option>)}</select><button className="map-generate-button" onClick={proceduralGenerate}><WandSparkles size={14}/> Generate Dungeon</button><button className="map-generate-button secondary" onClick={autoEncounterLogic}><Network size={14}/> Auto Encounter Logic</button></section>
    </aside>
    ...
    <aside className="map-right-panel">
      ...
      {!selectedRoom&&!selectedMarker&&!selectedProp&&<div className="map-empty-inspector"><MousePointer2 size={28}/><strong>Select something</strong><span>Rooms have direct resize handles. Combat rooms can own encounter logic and gates.</span></div>}
      ...
      {selectedMarker&&<MarkerInspector marker={selectedMarker} onPatch={patchMarker}/>} {selectedProp&&<PropInspector prop={selectedProp} onPatch={patchProp}/>}<section className="map-validation-panel"><div className="map-section-title">VALIDATION</div>{validation.ok?<div className="validation-ok"><Sparkles size={14}/> Layout, boss flow and encounter references are valid.</div>:validation.warnings.map((warning,index)=><div key={`${warning}-${index}`} className="validation-warning"><ShieldAlert size={13}/> {warning}</div>)}</section>
    </aside>
  </div>
}
