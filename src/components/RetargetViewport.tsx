...
export default function RetargetViewport({
  ...
  const poseRef = useRef({ landmarks, worldLandmarks, smoothing, mirrorX, showRig })
  const infoCallbackRef = useRef(onRigInfo)
  poseRef.current = { landmarks, worldLandmarks, smoothing, mirrorX, showRig }
  ...
  useEffect(() => {
    ...
    const grid = new THREE.GridHelper(14, 28, 0x2b394c, 0x17202c)
    scene.add(grid)
    ...
    const smooth = (source: PosePoint[]) => {
      if (source !== lastInput) {
        ...
        if (!smoothPoints || smoothPoints.length !== source.length) {
          smoothPoints = source.map((point) => ({ ...point }))
        } else {
          smoothPoints = source.map((point, index) => {
    ...
    const render = () => {
      const raw = poseRef.current.worldLandmarks?.length === 33
        ? poseRef.current.worldLandmarks
        : poseRef.current.landmarks
      if (runtime && raw?.length === 33) {
