...
export function forestArtDirectedSpeciesCrown(radius:number,height:number,variant:number,tier:number) {
const species = ((variant % 4) + 4) % 4
const pieces: THREE.BufferGeometry[] = []
const profiles = [
{ count: 12, spread: .9, vertical: 1.08, flat: .82, drift: .02 },
{ count: 11, spread: 1.28, vertical: .62, flat: 1.12, drift: -.03 },
{ count: 12, spread: .98, vertical: .9, flat: .98, drift: .04 },
{ count: 10, spread: 1.08, vertical: .78, flat: .72, drift: .34 },
] as const
...
geometry.translate(
  Math.cos(angle) * reach + directionBias,
...
const colors: number[] = []
for (let vertex = 0; vertex < position.count; vertex++) {
  const nx = position.getX(vertex) / Math.max(.001, radius)
  const ny = position.getY(vertex) / Math.max(.001, height)
  const nz = position.getZ(vertex) / Math.max(.001, radius)
  const directional = THREE.MathUtils.clamp(nx * -.08 + ny * .16 + nz * .045, -.1, .12)
  const shade = .82 + directional + tier * .018 + (clump % 3) * .025
  colors.push(shade * .76, shade, shade * .7)
}
geometry.setAttribute('color', new THREE.Float32BufferAttribute(colors, 3))
geometry.computeVertexNormals()
pieces.push(geometry)
