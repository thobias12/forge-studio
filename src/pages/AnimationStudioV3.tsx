...
export default function AnimationStudioV3() {
  ...
  useEffect(() => {
    ...
    void Promise.all([listAssets(), listAnimationDrafts()]).then(async ([items, savedDrafts]) => {
      ...
      if (savedDrafts[0]) await applyDraft(savedDrafts[0], nextCharacters)
  ...
  const publishCurrent = async (testAfter = false) => {
    ...
    setBusy(true)
    setPlaying(false)
    setStatus(testAfter ? `Publishing ${edit.name} and preparing the in-studio test…` : `Publishing ${edit.name} to ${characterAsset.name}…`)
    try {
      ...
      const nextPublished: PublishedState = { action: purpose, clipName: result.built.clipName, animationAssetId: result.animationAsset.id, set: result.set, revision: Date.now() }
      setPublished(nextPublished)
      if (selectedDraftId) {
        const saved = await saveAnimationDraft({
          id: selectedDraftId,
          motion: clip,
          settings: { ...draftSettings, publishedAnimationAssetId: result.animationAsset.id, publishedAt: new Date().toISOString() },
        })
        setDrafts((current) => current.map((draft) => draft.id === saved.id ? saved : draft))
        setDraftSavedAt(saved.asset.updatedAt)
      }
      if (testAfter) {
        ...
        setStatus(`${result.built.clipName} saved + published. Running the exact gameplay animation below.`)
        window.setTimeout(() => document.querySelector('.animation-studio-v3-test')?.scrollIntoView({ behavior: 'smooth', block: 'start' }), 120)
      } else {
        setStatus(`${result.built.clipName} published as ${selectedAction?.label}. The draft remains saved for later edits.`)
      }
      return true
    } catch (error) {
    ...
    <div className="animation-authoring-layout animation-authoring-v3">
      <section className="animation-authoring-main">
        ...
        <div className="animation-authoring-timeline">
          ...
          <div className="timeline-track character-track"><div className="track-label">CHARACTER</div><div className="track-clip has-character">{characterName} · {characterAsset ? 'publish target' : 'preview only'}</div></div>
        </div>
      </section>
      ...
      <aside className="animation-authoring-inspector">
        ...
        <div className="authoring-section publish-section">
          ...
          <div className="publish-summary"><div><span>Draft</span><strong>{selectedDraftId ? 'Autosaved' : 'Record first'}</strong></div><div><span>Character</span><strong>{characterAsset?.name ?? 'Select character'}</strong></div><div><span>Action</span><strong>{selectedAction?.label}</strong></div></div>
