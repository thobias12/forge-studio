...
import { Activity, CheckCircle2, Link2, Loader2, Pause, Play, RotateCcw, Save, Sparkles } from 'lucide-react'
...
export default function AnimationBindings() {
  ...
  useEffect(() => {
    ...
    }).catch(() => setStatus('Could not read the Shared Asset Library.'))
  ...
  const autoBind = () => {
    ...
    setSet(createAnimationSet(characterId, clips.map((entry) => entry.clip.name)))
  ...
  const save = async () => {
    ...
    try {
      ...
      await saveAsset({
        ...
        tags: ['animation-bindings', 'ForgeHumanoidV1', character.id],
        source: 'Forge Action Bindings',
        blob: animationSetBlob(set),
      })
      setStatus(`${mapped} gameplay action${mapped === 1 ? '' : 's'} bound to ${character.name}. Skillbound runtime will use these mappings automatically.`)
    } catch (error) {
  ...
  return <div className="animation-bindings-page">
    <header className="ab-toolbar">
      <div><span className="eyebrow">ANIMATION AUTHORING</span><h1>Action Bindings</h1><p>Tell Skillbound which clip means idle, movement, attack, hit, death and other gameplay actions.</p></div>
    ...
    <section className="ab-source-bar">
      ...
      <div className="ab-source-summary"><Link2 size={15}/><div><span>Target rig</span><strong>ForgeHumanoidV1</strong></div></div>
    </section>
    ...
    <div className="ab-workspace">
      <section className="ab-preview-panel">
        ...
        <div className="ab-preview-stage">{previewUrl && selectedClip ? <AnimationPreview src={previewUrl} clip={selectedClip} playing={playing} loop={selectedBinding?.loop ?? false} speed={selectedBinding?.speed ?? 1} onEnded={() => setPlaying(false)}/> : <div className="ab-preview-empty"><Play size={24}/><span>Select an action with a bound clip.</span></div>}</div>
        <div className="ab-preview-meta"><span>Action <b>{selectedAction}</b></span><span>Clip <b>{selectedBinding?.clip ?? 'Not bound'}</b></span><span>Speed <b>{(selectedBinding?.speed ?? 1).toFixed(2)}×</b></span></div>
      </section>
    ...
    <footer className="ab-status"><div>{status.includes('runtime') ? <CheckCircle2 size={14}/> : <RotateCcw size={14}/>}<span>{status}</span></div><strong>Gameplay cue → saved binding → exact GLB clip</strong></footer>
  </div>
}
