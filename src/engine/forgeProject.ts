...
export type ForgeRegionWorldGeneration = { size: ForgeRegionSize; elevation: number; cliffs: number; water: number; forestDensity: number; openSpace: number; exploration: number; loops: number; secretPaths: number; verticality: number; poiDensity: number; layerSeeds?: { terrain: number; routes: number; pois: number; dressing: number } }
