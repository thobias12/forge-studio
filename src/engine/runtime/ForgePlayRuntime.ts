...
function addRuntimePoiEnvironment(
  ...
  const addOuterStone = (
    ...
    runtimeBox(group, x, height * .5, z, .48, height, .42, darkStone)
    const marker = group.children[group.children.length - 1]
    marker.rotation.y = (random() - .5) * .28
  }
  ...
  if (poi.type === 'ruins' || poi.type === 'watchtower') {
    ...
    for (let row = 0; row < 2; row += 1) {
      for (let col = -2; col <= 2; col += 1) {
        const x = col * 1.35 + (row ? .35 : -.2)
        const z = 4.45 + row * 1.7 + (Math.abs(col) % 2) * .22
        runtimeBox(
          group,
          x,
          .5,
        ...
        const grave = group.children[group.children.length - 1]
        grave.rotation.y = (col - row) * .045
      }
    }
