...
import { Box, Check, Copy, FolderSync, Hammer, Image, PackagePlus, RotateCcw, Save, ScanLine, Sparkles, Sword, Upload, WandSparkles } from 'lucide-react'
...
export default function ItemForgeAutoFit({ onOpenModelCreator }: Props) {
  ...
  const refresh = async () => {
    ...
    let nextStatus = 'One master visual drives inventory, world drop and equipped presentation.'
  ...
  const autoFit = async (scope: ItemAutoFitScope) => {
    ...
    try {
      ...
      setFitProfile(fitted.profile)
      commit(fitted.workspace, `${scope === 'all' ? 'Auto Setup' : 'Auto Fit'} applied · detected ${fitted.profile.label}.`)
    } catch (error) {
  ...
  const createItem = async () => {
    ...
    const saved = commit(next, `${created.name} created. Forge is building and fitting a starter master model…`)
    setSelectedId(id)
    setMasterBusy(true)
    try {
  ...
  const createStarter = async () => {
    ...
    try {
      ...
      setAssets(await listAssets())
      await assignMasterAndFit(workspace, item.id, asset.id, isStarter ? 'Starter model regenerated.' : 'Starter master model created.')
    } catch (error) {
  ...
  if (!workspace || !item || !visual) return <div className="item-forge-loading"><Sword size={28}/><strong>Opening Item Forge</strong><span>{status}</span></div>

  return <div className="item-forge-page">
    <header className="item-forge-toolbar">
      <div><span className="eyebrow">SKILLBOUND / ITEM AUTHORING</span><strong>Item Forge</strong><small>One master model · automatic fitting · inventory · drop · equipped</small></div>
      ...
      <main className="item-forge-main">
        ...
        <section className={`item-master-workbench item-panel ${visual.masterAssetId ? 'has-master' : 'needs-master'}`}>
          <div className="item-master-heading">
            ...
            <div><span>MASTER 3D MODEL</span><strong>{masterAsset?.name ?? (visual.masterAssetId ? 'Assigned Library model' : 'No model needed beforehand')}</strong><small>{visual.masterAssetId ? 'Forge measures this model and can fit all three presentations automatically.' : 'Forge can create a starter, import your GLB, or hand off to Model Creator.'}</small></div>
            <b>{visual.masterAssetId ? 'READY' : 'CREATE'}</b>
          </div>

          <div className="item-master-actions item-master-actions-autofit">
            <button disabled={masterBusy} onClick={() => void createStarter()}><Sparkles size={16}/><span><strong>{!visual.masterAssetId ? 'Create starter model' : isStarter ? 'Regenerate starter' : 'Replace with starter'}</strong><small>Instant low-poly sword GLB</small></span></button>
          ...
          {fitProfile && <div className="item-fit-summary">
            <span><b>Detected</b>{fitProfile.label}</span>
          ...
          <p className="item-system-note"><Check size={13}/> Auto Setup is the baseline. Position, rotation and scale below are only for fine tuning.</p>
        </section>
