...
import {
  corridor, createStarterDungeon, dungeonBlob, generateDungeon, marker, room, validateDungeon,
...
const tools: Array<{ id: DungeonTool; label: string; icon: typeof MousePointer2 }> = [
  ...
  { id: 'corridor', label: 'Corridor', icon: Spline },
  { id: 'door', label: 'Door', icon: DoorOpen },
  { id: 'enemy', label: 'Enemy', icon: Skull },
  { id: 'loot', label: 'Loot', icon: Gem },
  { id: 'checkpoint', label: 'Checkpoint', icon: CircleDot },
  { id: 'portal', label: 'Portal', icon: Waypoints },
  { id: 'trigger', label: 'Trigger', icon: BoxSelect },
...
export default function MapStudio() {
  ...
  const [status, setStatus] = useState('Dungeon Builder ready. Click Room, then click the grid to place a room.')
  ...
  const onGroundClick = ({ x, z }: { x: number; z: number }) => {
    const gx = snap(x, value.gridSize)
    const gz = snap(z, value.gridSize)
    if (tool === 'room') {
      const nextRoom = room(`${titleCase(roomBrush)} Room`, roomBrush, gx, gz, roomBrush === 'boss' ? 12 : 7, roomBrush === 'boss' ? 11 : 7)
      mutate({ ...value, rooms: [...value.rooms, nextRoom] }, `${nextRoom.name} placed.`)
  ...
  const onRoomClick = (roomId: string) => {
    if (tool === 'erase') {
      mutate({
        ...value,
        rooms: value.rooms.filter((item) => item.id !== roomId),
        corridors: value.corridors.filter((item) => item.fromRoomId !== roomId && item.toRoomId !== roomId),
        markers: value.markers.filter((item) => item.roomId !== roomId),
      }, 'Room and attached dungeon data removed.')
      setSelectedRoomId(undefined)
      return
    }
    if (tool === 'corridor') {
      if (!corridorStartId) {
        setCorridorStartId(roomId)
        setStatus('Corridor start selected. Click another room to connect it.')
        ...
        if (!exists) mutate({ ...value, corridors: [...value.corridors, corridor(corridorStartId, roomId)] }, 'Rooms connected.')
  ...
  const onMarkerClick = (markerId: string) => {
    if (tool === 'erase') {
      mutate({ ...value, markers: value.markers.filter((item) => item.id !== markerId) }, 'Marker removed.')
      setSelectedMarkerId(undefined)
      return
    }
  ...
  const patchRoom = (patch: Partial<DungeonRoom>) => {
    ...
    mutate({ ...value, rooms: value.rooms.map((item) => item.id === selectedRoom.id ? { ...item, ...patch } : item) })
  }
  const patchMarker = (patch: Partial<DungeonMarker>) => {
    if (!selectedMarker) return
    mutate({ ...value, markers: value.markers.map((item) => item.id === selectedMarker.id ? { ...item, ...patch } : item) })
  }

  const proceduralGenerate = () => {
    const next = generateDungeon(Math.floor(Math.random() * 999999), value.theme)
    mutate(next, `${titleCase(value.theme)} dungeon generated. Edit any room or regenerate again.`)
  ...
  const saveToLibrary = async () => {
    await saveAsset({
      ...
      kind: 'file',
      mime: 'application/x-forge-dungeon+json',
      tags: ['map', 'dungeon', 'skillbound', value.theme],
      source: 'Forge Map Studio',
      blob: dungeonBlob(value),
    })
    setStatus(`${value.name} saved to the Shared Asset Library.`)
  }

  const exportSkillbound = () => {
    download(dungeonBlob(value), `${slug(value.name)}.forge-dungeon.json`)
    setStatus('Skillbound dungeon package exported.')
  }
    ...
    <div className="map-studio-page">
      <aside className="map-left-panel">
        ...
        <section className="map-panel-section">
          <div className="map-section-title">TOOLS</div>
        ...
        <section className="map-panel-section">
          <div className="map-section-title">ROOM PREFABS</div>
          <div className="map-prefab-list">
        ...
        <section className="map-panel-section compact">
          <div className="map-section-title">THEME</div>
          <select value={value.theme} onChange={(e) => mutate({ ...value, theme: e.target.value as DungeonTheme })}>{themes.map((item) => <option key={item} value={item}>{titleCase(item)}</option>)}</select>
          <button className="map-generate-button" onClick={proceduralGenerate}><WandSparkles size={14} /> Generate Dungeon</button>
        </section>
      </aside>

      <main className="map-workspace">
        <header className="map-toolbar">
          <div className="map-name-block"><span>SKILLBOUND / DUNGEON</span><input value={value.name} onChange={(e) => setValue({ ...value, name: e.target.value })} /></div>
          <div className="map-toolbar-actions">
            ...
            <button onClick={proceduralGenerate}><RotateCcw size={14} /> Regenerate</button>
            <button onClick={() => void saveToLibrary()}><Save size={14} /> Save</button>
            <button className="primary" onClick={exportSkillbound}><Download size={14} /> Export Skillbound</button>
          </div>
        </header>

        <div className="map-viewport-wrap">
          <DungeonViewport value={value} tool={tool} selectedRoomId={selectedRoomId} selectedMarkerId={selectedMarkerId} corridorStartId={corridorStartId} topDown={topDown} playtest={playtest} onGroundClick={onGroundClick} onRoomClick={onRoomClick} onMarkerClick={onMarkerClick} />
          <div className="map-viewport-hud top-left"><strong>{titleCase(tool)}</strong><span>{toolHint(tool, corridorStartId)}</span></div>
          <div className="map-viewport-hud bottom-left"><span>{value.rooms.length} rooms</span><b>·</b><span>{value.corridors.length} corridors</span><b>·</b><span>{value.markers.length} gameplay markers</span></div>
        ...
        <section className={`map-flow ${flowOpen ? 'open' : ''}`}>
          <button className="map-flow-header" onClick={() => setFlowOpen(!flowOpen)}><Network size={14} /><strong>DUNGEON FLOW</strong><span>{flowOpen ? 'Hide' : 'Show'}</span></button>
          {flowOpen && <div className="map-flow-body">
            {value.rooms.map((roomItem, index) => <div key={roomItem.id} className={`flow-node ${roomItem.type} ${selectedRoomId === roomItem.id ? 'selected' : ''}`} onClick={() => { setSelectedRoomId(roomItem.id); setSelectedMarkerId(undefined); setTool('select') }}><span>{roomItem.type}</span><strong>{roomItem.name}</strong>{index < value.rooms.length - 1 && <ChevronRight size={14} />}</div>)}
          </div>}
        </section>
      ...
      <aside className="map-right-panel">
        ...
        {selectedRoom && <RoomInspector room={selectedRoom} onPatch={patchRoom} />}
        {selectedMarker && <MarkerInspector marker={selectedMarker} onPatch={patchMarker} />}
        <section className="map-validation-panel">
          <div className="map-section-title">VALIDATION</div>
          {validation.ok ? <div className="validation-ok"><Sparkles size={14} /> All rooms are connected and the dungeon has an entrance and boss.</div> : validation.warnings.map((warning) => <div key={warning} className="validation-warning"><ShieldAlert size={13} /> {warning}</div>)}
        </section>
      </aside>
    </div>
  )
...
function MarkerInspector({ marker, onPatch }: { marker: DungeonMarker; onPatch: (patch: Partial<DungeonMarker>) => void }) {
  ...
  const patchData = (key: string, value: string | number | boolean) => onPatch({ data: { ...data, [key]: value } })
  return <>
    <section className="map-inspector-section"><div className="map-section-title">{marker.type.toUpperCase()}</div><label>Name<input value={marker.name} onChange={(e) => onPatch({ name: e.target.value })} /></label><div className="map-number-grid"><NumberField label="X" value={marker.x} onChange={(x) => onPatch({ x })} /><NumberField label="Z" value={marker.z} onChange={(z) => onPatch({ z })} /></div></section>
...
function NumberField({ label, value, min, step = 0.5, onChange }: { label: string; value: number; min?: number; step?: number; onChange: (value: number) => void }) {
  return <label>{label}<input type="number" value={Number(value.toFixed(2))} min={min} step={step} onChange={(e) => onChange(Number(e.target.value))} /></label>
}

function nearestRoom(rooms: DungeonRoom[], x: number, z: number) {
  return [...rooms].sort((a, b) => distanceSq(a.x, a.z, x, z) - distanceSq(b.x, b.z, x, z))[0]
}
...
function isMarkerTool(tool: DungeonTool): tool is Extract<DungeonTool, 'door' | 'enemy' | 'loot' | 'checkpoint' | 'portal' | 'trigger' | 'light'> { return ['door', 'enemy', 'loot', 'checkpoint', 'portal', 'trigger', 'light'].includes(tool) }
function slug(value: string) { return value.trim().replace(/[^a-z0-9_-]+/gi, '-').replace(/^-|-$/g, '').toLowerCase() || 'skillbound-dungeon' }
function download(blob: Blob, filename: string) { const url = URL.createObjectURL(blob); const anchor = document.createElement('a'); anchor.href = url; anchor.download = filename; document.body.appendChild(anchor); anchor.click(); anchor.remove(); window.setTimeout(() => URL.revokeObjectURL(url), 1000) }
...
function toolHint(tool: DungeonTool, corridorStartId?: string) {
  ...
  if (tool === 'corridor') return corridorStartId ? 'Click the destination room.' : 'Click the first room, then another room.'
