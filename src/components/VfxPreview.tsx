...
function createRuntime(emitter:ForgeVfxEmitter,scene:THREE.Scene):EmitterRuntime{
  ...
  const styleValue=emitter.style==='soft'?0:emitter.style==='spark'?1:emitter.style==='square'?2:emitter.style==='ring'?3:emitter.style==='diamond'?4:5
  const material=new THREE.ShaderMaterial({ transparent:true,depthWrite:false,blending:emitter.blendMode==='additive'?THREE.AdditiveBlending:THREE.NormalBlending,vertexColors:true,uniforms:{ uPixelRatio:{value:Math.min(devicePixelRatio,2)},uStyle:{value:styleValue}},vertexShader:`attribute vec3 aColor;attribute float aSize;attribute float aAlpha;varying vec3 vColor;varying float vAlpha;uniform float uPixelRatio;void main(){vColor=aColor;vAlpha=aAlpha;vec4 mv=modelViewMatrix*vec4(position,1.0);gl_PointSize=max(1.0,aSize*uPixelRatio*(320.0/max(0.2,-mv.z)));gl_Position=projectionMatrix*mv;}`,fragmentShader:`varying vec3 vColor;varying float vAlpha;uniform float uStyle;void main(){vec2 p=gl_PointCoord*2.0-1.0;float a=1.0;if(uStyle<0.5){a=1.0-smoothstep(0.15,1.0,length(p));}else if(uStyle<1.5){a=1.0-smoothstep(0.0,1.0,abs(p.x)+abs(p.y)*0.32);}else if(uStyle<2.5){a=step(max(abs(p.x),abs(p.y)),1.0);}else if(uStyle<3.5){a=1.0-smoothstep(0.035,0.18,abs(length(p)-0.62));}else if(uStyle<4.5){a=1.0-smoothstep(0.68,1.0,abs(p.x)+abs(p.y));}else{float axis=min(abs(p.x),abs(p.y));float diag=min(abs(p.x+p.y),abs(p.x-p.y))*0.7071;float rays=1.0-smoothstep(0.055,0.23,min(axis,diag));float fade=1.0-smoothstep(0.52,1.05,length(p));a=rays*fade;}if(a<0.01)discard;gl_FragColor=vec4(vColor,vAlpha*a);}` })
