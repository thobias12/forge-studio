...
export function dungeonProps(value: DungeonWithProps) {
  // Crypt auto-dressing now lives in the shared environment renderer so Editor,
  // Walk and ARPG all get the same detailed architecture instead of duplicate
  // primitive pillars/statues. Explicitly placed props still render normally.
  return value.props ?? []
}
